import { signInWithProvider } from '../useAuth'
import Logo from './Logo'

function GoogleIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 48 48" aria-hidden="true">
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.7-6.1 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.1 8 3l5.7-5.7C34.6 6.1 29.6 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.7-.4-3.5z"
      />
      <path
        fill="#FF3D00"
        d="M6.3 14.7l6.6 4.8C14.6 15.9 18.9 13 24 13c3.1 0 5.8 1.1 8 3l5.7-5.7C34.6 6.1 29.6 4 24 4c-7.3 0-13.6 4.1-16.9 10.1z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.5 0 10.4-2.1 14.1-5.5l-6.5-5.5c-2 1.5-4.6 2.4-7.6 2.4-5.2 0-9.6-3.3-11.3-7.9l-6.6 5.1C9.4 39.8 16.1 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.2 4.2-4.1 5.6l6.5 5.5C41.8 35.9 44 30.5 44 24c0-1.3-.1-2.7-.4-3.5z"
      />
    </svg>
  )
}

function LoginScreen() {
  return (
    <div className="min-h-screen flex items-center justify-center p-20">
      <div className="main-btn bg-white p-30 flex flex-col gap-20 w-full max-w-[400px]">
        <div className="flex flex-col gap-15">
          <Logo className="text-46" />
          <p className="text-14 font-title text-neutral-500 text-center">
            Sign in to save your recipes, plans and profile to your account.
          </p>
        </div>

        <button
          type="button"
          onClick={() => signInWithProvider('google')}
          className="main-btn font-title font-bold px-15 py-15 flex items-center justify-center gap-10"
        >
          <GoogleIcon />
          Continue with Google
        </button>
      </div>
    </div>
  )
}

export default LoginScreen
